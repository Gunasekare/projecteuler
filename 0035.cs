//Problem - Euler Problem #35 (Circular primes)
...
namespace _0035
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<int> primesList = prime.PrimesBelowEqN(limit);
            List<int> circularPrimes = new List<int>();

            foreach (int primeNum in primesList)
            {   
                if (circularPrimes.Contains(primeNum))
                    continue;

                List<int> primePermutations = new List<int>(){primeNum};
                bool isPrime = true;
                string value = primeNum.ToString();

                for (int i = 0; i < (value.Length - 1); ++i)
                {
                    value = new string(value.Substring(1) + value[0]);
                    if (!primesList.Contains(int.Parse(value)))
                    {
                        isPrime = false;
                        break;
                    }
                    primePermutations.Add(int.Parse(value));
                }

                if (isPrime)
                    foreach (int number in primePermutations)
                        if (!circularPrimes.Contains(number))
